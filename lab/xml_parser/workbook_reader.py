...
Reads 5471/8858 compliance workbooks and extracts structured per-entity
...
class WorkbookReader:
    ...
    def _parse_layout_a(self, schedule_key: str) -> pd.DataFrame:
        ...
        entity_codes = []
        import re
        _entity_pat = re.compile(r"^(C|SS|AM)\d{2,4}$")
        for i in range(data_start_col - 1, len(header_row)):
            code = str(header_row[i] or "").strip()
            if code and _entity_pat.match(code):
                entity_codes.append((i, code))
            elif code and not _entity_pat.match(code) and len(entity_codes) > 0:
                break
    ...
    def _parse_cng_ep_gilti(self) -> WorkbookData:
        """Parse Layout-B-style E&P/GILTI/SubF workbook (Foreign Summary sheet)."""
        data = WorkbookData(source_path=self.path)
