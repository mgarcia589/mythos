"""Schedule H (Current E&P) extraction rules for Form 8858.

Maps OIT PDF line descriptions to canonical XML field names for
IRS Form 8858 Schedule H — Current Earnings and Profits of FDE/FB.

8858 Sch H uses the same structure as 5471 Sch H: single-amount
lines for E&P computation, with an exchange rate line.

XML structure:
  <IRS8858ScheduleH>
    <ForeignCYNetIncomePerBooksAmt>...</ForeignCYNetIncomePerBooksAmt>
    <CurrentEarningsAndProfitsAmt>...</CurrentEarningsAndProfitsAmt>
    <CurrEarnAndPrftInUSDollarsAmt>...</CurrEarnAndPrftInUSDollarsAmt>
...
SCH8858H_LINE_ITEMS: list[tuple[str, str, str, str]] = [
    ...
    ("5d", r"e.?p.*in\s*u\.?s|e.?p.*usd|in\s*u\.?s\.?\s*dollar|in\s*us\s*dollar|earnings.*profits.*in\s*u", "CurrEarnAndPrftInUSDollarsAmt", "Current E&P in USD"),
    ("5a", r"current\s*e.?p(?!\s*in\s*u)(?!\s*after)(?!\s*plus)|current\s*earnings\s*and\s*profits(?!\s*(?:in|or\s*tax))", "CurrentEarningsAndProfitsAmt", "Current E&P"),
...
_COMPILED_PATTERNS: list[tuple[re.Pattern, str, str, str]] = [
    ...
    for line_num, pattern, xml_field, desc in SCH8858H_LINE_ITEMS
]

ALL_FIELDS: list[tuple[str, str, str]] = [
    (xml_field, line_num, desc)
    for line_num, _, xml_field, desc in SCH8858H_LINE_ITEMS
]
...
def identify_line_item(description: str) -> Optional[tuple[str, str, str]]:
    """Identify a Form 8858 Schedule H line item from its PDF description.
